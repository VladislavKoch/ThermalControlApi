...
public interface MeasurementRepository extends JpaRepository<Measurement, Integer> {
    ...
    @Query(value = "SELECT row_number() OVER () AS id, date_trunc('hour', time) - (EXTRACT(HOUR FROM time) % 3) * " +
            "interval '1 hour' AS time, AVG(temperature) AS temperature, AVG(humidity) AS humidity, " +
            "AVG(sensor) AS sensor FROM measurement GROUP BY time ORDER BY time;", nativeQuery = true)
    ...
    @Query(value = "SELECT row_number() OVER () AS id, date_trunc('hour', time) - (EXTRACT(HOUR FROM time) % 3) * " +
            "interval '1 hour' AS time, AVG(temperature) AS temperature, AVG(humidity) AS humidity, " +
            "AVG(sensor) AS sensor FROM measurement where sensor = ?1 GROUP BY time ORDER BY time;", nativeQuery = true)
