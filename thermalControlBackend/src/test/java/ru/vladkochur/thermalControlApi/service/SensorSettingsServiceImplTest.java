...
class SensorSettingsServiceImplTest {
    ...
    private final Sensor sensor = DataUtils.getYaroslavlSensorPersisted();

    @Test
    @DisplayName("Test get all settings functionality")
    public void givenTwoSettings_whenGetAll_thenRepositoryIsCalled() {
        //given
        List<SensorSettings> settings = List.of(DataUtils.getFirstSensorSettingsPersisted(sensor),
                DataUtils.getFirstSensorSettingsPersisted(sensor));
    ...
    public void givenExistedSettings_whenSetSettingsBySerial_thenRepositoryIsCalled() {
        ...
        SensorSettings settings = DataUtils.getFirstSensorSettingsTransient(sensor);
        SensorSettings settings2 = DataUtils.getFirstSensorSettingsPersisted(sensor);
